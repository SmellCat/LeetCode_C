...
int modbook()
{  // 修改除id之外的信息  因为要修改id 直接删掉后重新添加会更好
	system(CLEAR);	///////////////////////////////////////////
	Book *bp = NULL;
	while (1) {
		int id = 0;
				...
				switch (i) {
					...
					case 3:
						while (1) {
							...
							if (c == 'Y' || c == 'y') {
								status == AVAILABLE ? bp->status = UNAVAILABLE :
													  (bp->status = AVAILABLE);
								printf("修改成功\n");
								return id;
							}
	...
	if (bp != NULL) {
		while (1) {
			...
			if (YN == 'Y' || YN == 'y') {
				...
				free(
					bp->name);	// 2018-05-26修改：添加释放name这块内存//bp内存的只是指向name的指针
				free(bp);
				bp = NULL;
				printf("图书注销成功\n");
				break;
